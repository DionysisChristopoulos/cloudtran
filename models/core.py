...
# Lint as: python3
"""Core components of CloudTran.

Consists of:

1. Timeseries Encoder.
2. Outer Decoder.
3. Inner Decoder.
"""
...
class TimeseriesEncoder(layers.Layer):
  """Encodes timeseries cube consisting of T consecutive image patches into an aggregated spatial context per channel.
  Produces a parallel auxilliary output.

  Consists of a stack of row/column attention layers.
  """

  def __init__(self, config, **kwargs):
    super(TimeseriesEncoder, self).__init__(**kwargs)
    self.config = config
  ...
  def build(self, input_shapes):
    ...
    self.pos_embed = tran_layers.PositionEmbed(axes=3, max_lengths=num_channels)
    ...
    if self.config.aggregation == 'attention':
      ...
      for layer_ind in range(self.config.num_temp_layers):
        umask_temp = tran_layers.SelfAttentionND(
      ...
      self.project_aggr = tran_layers.DenseND(
  ...
  def call(self, inputs, channel_index=None, training=True):
    ...
    if channel_index is not None:
      ...
      logits=[[] for _ in range(self.n_channels)]
    index = 0
    ...
    for channel_ind in range(num_channels):
      ...
      if channel_index is not None:
          ...
          index = channel_ind % self.n_channels
          channel += 256 * (channel_ind % self.n_channels)

      channel = tf.expand_dims(channel, axis=-1)
      channel = tf.one_hot(channel, depth=256 * self.n_channels)
    ...
    for ind in range(len(logits)):
      ...
      if self.config.aggregation == 'conv':
        ...
        for layer, norm in zip(self.temp_layers, self.layer_norms):
          ...
          temp_output = tran_layers.residual_dropout(
...
class OuterDecoder(layers.Layer):
  ...
  def build(self, input_shapes):
    ...
    self.pos_embed = tran_layers.PositionEmbed(axes=[1, 2], max_lengths=res)

    self.residual_layers, self.layer_norms, self.cmlp_layers = [], [], []
    num_norms = self.config.num_outer_layers * 4
    if self.cond_ln:
      for _ in range(num_norms):
        curr_norm = tran_layers.ConditionalLayerNorm(
    ...
    for layer_ind in range(self.config.num_outer_layers):
      # unmasked row
      unmask_row = tran_layers.SelfAttentionND(
      ...
      # masked column,
      mask_col = tran_layers.SelfAttentionND(
    ...
    self.shift_down = tran_layers.Shift(dimension=0, resolution=res)
  ...
  def call(self, inputs, training=True):
    ...
    for layer, norm in zip(self.residual_layers, self.layer_norms):
      ...
      output = tran_layers.residual_dropout(
...
class InnerDecoder(layers.Layer):
  ...
  def build(self, input_shapes):
    ...
    self.pos_embed = tran_layers.PositionEmbed(axes=[1, 2], max_lengths=res)
    self.shift_right = tran_layers.Shift(dimension=1, resolution=res)
    ...
    for layer_ind in range(self.config.num_inner_layers):
      ...
      mask_row = tran_layers.SelfAttentionND(
          hidden_size=hidden_size, num_heads=num_heads, mask='future',
          nd_block_size=[1, width], resolution=[height, width],
          cond_q=self.cond_att_q,
          cond_k=self.cond_att_k,
